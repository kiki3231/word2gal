...
- **立绘绿幕抠图为准（防白边）**：半身立绘默认绿幕 `#00FF00` + `cut-sprite --mode green`；残底（绿/灰/白边/棋盘格）不上架
- **路人/OC 画风锚**：原作不存在或无名路人，必须用本篇已定原作角色立绘作 `reference_image` 画风锚，禁止另起写实/异画风；同篇画风 0 差异
- **立绘不串脸**：`speakerToId` 覆盖全部说话人；缺差分只回退同角色 `neutral`，禁止回退到其他角色或全局 default
- **生图一律自动确认**：用户提交文章并进入制作后，立绘/背景/所需差分的文生图**全部自动执行、自动确认**，禁止每张图都停下来让用户点确认或口头询问「要不要生成」；批量连出，出错按重试规则自行处理
- 禁止改写播放器核心 JS；只替换 `__SCRIPT_JSON__` / `__ASSETS_JSON__`（历史面板等功能改在 `templates/player-basic.html` 模板内）
...
- **立绘透明路径（绿幕默认）**：  
  1. 按风格包**绿幕**路径出半身立绘（`#00FF00`）+ 参考形象/画风锚  
  2. `cut-sprite.mjs --mode green` 抠净（浅灰白底急救才用 `--mode flood`）  
  3. `check-sprite-alpha.mjs` 抽检 + **Agent 目视**：无绿/白/灰毛边、眼睛与发丝残底  
...
- [ ] 立绘绿幕抠净；`check-sprite-alpha` 通过；目视无绿/白/灰边与眼睛发丝残底  
- [ ] `speakerToId` 覆盖全部说话人；历史面板可看【说话人】+正文  
- [ ] 路人/OC 与同篇原作角色画风一致（有画风锚参考）  
- [ ] 表情差分集合能对应文章情绪成色  
